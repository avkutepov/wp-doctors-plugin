...
1. Установите и активируйте плагин `Doctors CPT`
...
- Type hints (`\WP_Query`, `int`, `string`, `bool`) для типобезопасности
...
**Исключения:**
Если врач работает в нескольких местах (например, в поселке и областном центре), можно включить иерархию, изменив параметр в `doctors-plugin.php`:
...
**Важно:** `flush_rewrite_rules()` - "тяжелая" операция, поэтому используется только при активации/деактивации, а не при каждой загрузке страницы.
